...
public class Semant {
  ...
  public Semant(Frame.Frame frame, ErrorMsg.ErrorMsg err) {
    this(new Env(), new Level(frame));
  }
  Semant(Env e, Level l) {
    env = e;
    level = l;
  }
  private BoolList makeBoolList(boolean[] escapes) {
    BoolList list = null;
    for (int i = escapes.length - 1; i >= 0; i--) {
      list = new BoolList(escapes[i], list);
    }
    return list;
  }

  public void transProg(Absyn.Exp exp) {
     new FindEscape.FindEscape((VarExp) exp);
  ...
  Exp transDec(Absyn.FunctionDec d) {
    ...
    for (Absyn.FunctionDec f = d; f != null; f = f.next) {
      ...
      Level newLevel = new Level(level, f.name, makeBoolList(escape(f.params)), f.leaf);
  ...
  private boolean[] escape(FieldList params) {
